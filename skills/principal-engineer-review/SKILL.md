...
Use this skill when the user asks for a PR review, asks whether a change is the right approach, wants a principal engineer perspective, or asks for critique before shipping.
...
Do not use this for PR body writing or review-comment follow-up unless that is part of the request; use a dedicated PR-handoff workflow for PR preparation and review-response.
...
3. If a PR-handoff skill is available, use it when preparing a branch for PR, writing or updating the PR body, handling review comments, or deciding whether the branch is ready for review.
4. If a personal voice/style skill is available, use it when drafting review comments that should match the author's voice; otherwise use a neutral, professional tone.
...
4. Check whether the implementation is the minimum viable code that solves the problem safely.
5. Evaluate long-term maintenance: future readability, clear ownership, obvious rollback, testable boundaries, hidden coupling, and whether future teams will have to support abstractions created prematurely.
6. Evaluate security and privacy: authorization before data access, tenant boundaries, customer data exposure, unsafe logging, secret handling, injection paths, dependency risk, and success-shaped fallbacks.
7. Evaluate reliability and operations: idempotency, retries, timeouts, resource usage, migrations, partial failure, deploy order, monitoring, and rollback.
8. Evaluate whether the change is over-optimized: premature generalization, speculative extension points, caching without evidence, batching without need, complex config, or abstractions built for imagined future cases.
9. Decide whether each concern is blocking, should be fixed before merge, can be documented as a follow-up, or should be ignored.
10. If a miss reveals invisible context, identify the durable artifact that would prevent the same miss next time.
...
This skill is grounded in its existing workflow instructions and conventions. It captures the reusable workflow for principal-engineer code review focused on correctness, security, data integrity, operational risk, and MVP scope.
