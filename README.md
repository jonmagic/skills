...
- [roundtable](skills/roundtable/) — Run an explicitly requested multi-model review or adjudicate a consequential disagreement between a driver and reviewer. Uses one different-provider reviewer by default and a third provider only when direct verification cannot resolve the dispute. Not for routine ambiguity or fleet mode.
